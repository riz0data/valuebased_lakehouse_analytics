...
Every metric in `_metrics.yml` carries a `meta` block: `domain` (which of
Payment Integrity, Risk Adjustment, Clinical Quality, or Pharmacy it
belongs to), `unit` (usd, percent, days, count, and so on), `synonyms`
(alternate names stakeholders actually use, like "risk score" for
`raf_score`), and `data_maturity` (`real_data` or `synthetic`, matching
the honesty notes already in each metric's `description`). This is what
lets an AI agent or a text-to-SQL tool disambiguate a loosely-phrased
question against the right governed metric instead of guessing at a raw
join - see ADR-006.

An AI agent connects to this project through the
[dbt MCP server](https://github.com/dbt-labs/dbt-mcp), configured via
`.env.example` (copy to `.env`, never commit the real one) and
`.mcp/mcp.json.example`. Two tiers, matching dbt-mcp's own design:

- **dbt CLI tools** (run, build, test, compile, list resources) work
  today against this project's local dbt Core setup - no dbt Cloud
  account needed, just `DBT_PROJECT_DIR` and `DBT_PATH` set.
- **Semantic Layer and Discovery API tools** (`list_metrics`,
  `get_dimensions`, `query_metrics`, model lineage) require a real dbt
  Platform / dbt Cloud account with this project connected - this is
  the tier that lets an agent actually query the 24 governed metrics
  above by name.

Raw SQL tools (`execute_sql`, `text_to_sql`) are disabled
(`DISABLE_SQL=true`), which is also dbt-mcp's own default. An agent
connected to this project can query governed metrics; it cannot write
its own SQL against Gold or Vault tables directly. See ADR-006 for why
that's a deliberate governance boundary, not a missing feature.

**A note on what `DISABLE_SQL` actually does.** Even with SQL tools
disabled, an agent calling `query_metrics` still runs a real query
against real Gold-layer tables in the warehouse - nothing here is
mocked or pre-computed. What's disabled is *who writes the SQL*. With
`query_metrics`, the SQL is generated by MetricFlow from a metric
definition a human already wrote and reviewed once, in `_metrics.yml`.
With `execute_sql` or `text_to_sql` enabled, the agent would generate
that SQL itself, from scratch, picking its own joins and filters for
each question, with no guarantee it matches how the metric is defined
everywhere else.

This is a deliberate default for this project, not a hard limit of the
technology. Setting `DISABLE_SQL=false` is a legitimate choice for
exploratory, ad hoc questions that don't map to an existing metric,
where a human is expected to sanity-check the result before trusting
it. A common pattern is to run both modes side by side: governed
`query_metrics` calls for anything that feeds a dashboard or a
decision, and `execute_sql` open only for one-off, human-supervised
exploration. This repo ships with the stricter default because it's
demonstrating the trustworthy-by-construction path, not because the
more permissive path is unsafe in every context.

Row and column-level access control for who (or what identity) can see
which rows and columns once a query - human, BI tool, or AI agent -
reaches the warehouse is enforced at the Unity Catalog layer underneath
the Semantic Layer, not duplicated here. See
`docs/decisions/ADRs.md` (ADR-007) and `dbt/models/gold/README.md` for
the row filter and column mask policies applied to Gold tables.

What an agent actually *does* once connected - which tool calls it
makes, which metric a question resolves to, whether it succeeds, and
whether it's behaving normally - is captured by the continuous
Delta-table audit logging and rule-based alerting in
`../../observability/` (ADR-008): a 1-minute scheduled job logs every
tool call, and a companion job evaluates error rate, stuck-loop, and
latency-spike rules against it in near real time. No agent is live
against this project yet, so that folder is a reference implementation
plus a runnable, dependency-free simulation, not captured production
traffic.
