...
[![CI](https://github.com/REPLACE_WITH_YOUR_USERNAME/value-based-care-lakehouse/actions/workflows/ci.yml/badge.svg)](https://github.com/REPLACE_WITH_YOUR_USERNAME/value-based-care-lakehouse/actions/workflows/ci.yml)
...
+-- .github/workflows/ci.yml        # ingestion tests + dbt structure checks on every PR
+-- .github/ci-profiles/profiles.yml  # dbt-databricks connection profile (no real secrets - env_var() only)
+-- scripts/validate_dbt_structure.py  # standalone ref/YAML validator, used by CI and by hand
```
...
**CI (`.github/workflows/ci.yml`)** runs in two tiers, and is honest about
which one actually exercises on a fork with zero setup:

1. `ingestion-tests` and `dbt-structure` run on every push/PR with no
   credentials at all - every ingestion module's test suite against its
   committed sample fixtures, plus a full `dbt parse` and a custom
   ref/YAML structural check (`scripts/validate_dbt_structure.py`) of
   every model in `/dbt`. This is the tier that actually runs today.
2. `dbt-build-databricks` runs a real `dbt build` against a live
   Databricks SQL Warehouse, but only if the repo has a
   `DATABRICKS_CONFIGURED` repository variable set to `true` and the
   `DATABRICKS_HOST` / `DATABRICKS_HTTP_PATH` / `DATABRICKS_TOKEN`
   secrets configured - this portfolio repo doesn't keep a warehouse
   running permanently, so this job reports as skipped rather than
   failed until you provide your own.

**Databricks Asset Bundle (`databricks.yml`)** defines two jobs -
`ingestion_job` (lands all 7 ingestion modules' sample data into Bronze
Delta tables) and `dbt_build_job` (triggers `ingestion_job`, then runs
`dbt deps && dbt build && dbt test` against a SQL Warehouse). To deploy
...
You'll need to replace the placeholder `workspace.host` values in
`databricks.yml`'s `dev`/`prod` targets and the `warehouse_id` variable
with your own Databricks SQL Warehouse details first.
